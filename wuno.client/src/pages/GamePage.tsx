...
export default function GamePage() {
  const { code } = useParams<{ code: string }>(); // route like /game/:code
  ...
  const myUserId = useMemo(() => {
      // Registered users come from HttpOnly cookie (Auth.me backend). For guests, use your guest cookie.
    ...
    (async () => {
      try {
        ...
        hub.on("WordChanged", (word: string) => {
          // server relays others’ typing; we don’t get seat, so map to current turn’s seat (the typist)
          setState(s => {
            if (!s) return s;
        ...
        await hub.invoke("ConnectToGame", code, myUserId);
  ...
  const setReady = async (ready: boolean) => {
    ...
    try {
      await hubRef.current.invoke("Ready", state.gameId, meSeat, ready);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const onLocalType = (seat: number, word: string) => {
    ...
    // best-effort; backend throttles with TypingGate
