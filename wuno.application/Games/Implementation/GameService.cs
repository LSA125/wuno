...
namespace Wuno.Application.Games.Implementation
{
    public sealed class GameService(AppDbContext db, IWordList wl, ICodeGeneratorService cg, ITurnTimer tt) : IGameService
    {
        ...
        async public Task<LeaveGameResult> LeaveGameAsync(Guid userId, CancellationToken ct)
        {
            ...
            if (player == null)
            {
                throw new Exception("User has no active player");
            }
            ...
            if (connectedCount <= 1)
            {
                ...
                if (game != null && game.Status != GameStatus.FINISHED)
                {
                    ...
                    // Build final game state for MatchEnded broadcast
                    var players = game.Players
                        .Where(p => p.IsTaken)
                        .Select(State.PlayerToState)
                        .ToList();
                    var finalState = State.GameToState(game,
                        players,
                        game.CurrentRound != null ? State.RoundToState(game.CurrentRound) : null,
                        game.CurrentTurn != null ? State.TurnToState(game.CurrentTurn) : null);
                    
                    return new LeaveGameResult(gameId, GameEnded: true, RemainingPlayers: null, FinalState: finalState);
                }
            }
            
            await _db.SaveChangesAsync(ct);
            
            // Game continues - return remaining players for broadcast
            var remainingPlayers = await GetPlayersAsync(gameId, ct);
            return new LeaveGameResult(gameId, GameEnded: false, RemainingPlayers: remainingPlayers, FinalState: null);
        }
